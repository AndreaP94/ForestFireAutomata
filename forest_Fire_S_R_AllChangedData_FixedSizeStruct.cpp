...
int main(int argc, char** argv) {
	...
	while(currentIteration < 1000)
	{
		...
		if(!iAmTheMaster) {
			...
			MPI_Send(&leftProcessValueChangedToSend[0], BLOCKCOLS, cellOfNeighbourdProcess, left, 95, linearArrayTopology);
			MPI_Send(&rightProcessValueChangedToSend[0], BLOCKCOLS, cellOfNeighbourdProcess, right, 91, linearArrayTopology);

			MPI_Recv(&rightProcessValueChangedToSend[0], BLOCKCOLS, cellOfNeighbourdProcess, right, 95, linearArrayTopology, MPI_STATUS_IGNORE);
			MPI_Recv(&leftProcessValueChangedToSend[0], BLOCKCOLS, cellOfNeighbourdProcess, left, 91, linearArrayTopology, MPI_STATUS_IGNORE);

			for(int j = 0; j < BLOCKCOLS; j++) {
				neighbourdDataOfRightProcess[rightProcessValueChangedToSend[j].position] = rightProcessValueChangedToSend[j].newValue;
				neighbourdDataOfLeftProcess[leftProcessValueChangedToSend[j].position] = leftProcessValueChangedToSend[j].newValue;
			}
		...
		if(iAmTheMaster) {
			int dim = BLOCKROWS * BLOCKCOLS;
			if(enableGraphic)
				al_peek_next_event(queue, &event);

			for(int i = 1; i < numberOfProcess; i++) {
				modifiedCellOfCurrentGeneration mod[dim];
				MPI_Recv(&mod, dim, cellOfCurrentGeneration, i, 99, MPI_COMM_WORLD, &status);
